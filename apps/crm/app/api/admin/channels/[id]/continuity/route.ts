...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!['COEXISTENCE', 'YCLOUD_COEXISTENCE'].includes(channel.connectionMode) || !channel.isOnBizApp || channel.platformType !== 'CLOUD_API') {
      throw new CrmApiError(409, 'COEXISTENCE_NOT_CONFIRMED', 'Primero el proveedor debe validar la conexión de Coexistence para este número.')
    }
