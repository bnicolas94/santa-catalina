...
4. **Depósito de un local:**
   Al informarlo, el importe sale de la Caja Chica y entra inmediatamente en la caja activa de la misma sede marcada como `recibeDepositos` (Caja Fuerte). El sobre queda pendiente de validación dentro de esa Caja Fuerte. El local no puede declarar un importe mayor al disponible en Caja Chica; la excepción exige el email y la contraseña de un ADMIN activo y deja una auditoría con el saldo, el monto y el administrador que autorizó. Al validar el sobre, cualquier diferencia se registra con contrapartidas en Caja Chica y Caja Fuerte. El administrador puede transferir el monto real desde la Caja Fuerte hacia otra caja o dejarlo allí; en este último caso no se crea otra transferencia ni se suma un segundo ingreso. Los depósitos históricos sin caja de recepción conservan su conciliación anterior y sólo pueden validarse con el destino anterior.

Los indicadores diarios de la pantalla de Caja muestran únicamente ingresos y egresos externos. Las transferencias internas y todos los movimientos vinculados a depósitos se conservan en el historial y modifican las cajas correspondientes, pero no inflan esos indicadores.
...
El servidor solicita y descarga reportes settlement directamente de la API de la cuenta conectada. `npm start` inicia Next y activa el worker de `instrumentation.ts`; no se ejecuta durante el build ni en el CRM. Requiere un servidor Node persistente (Railway, sin suspensión por inactividad). `MP_REPORTES_AUTO=false` pausa el worker. No requiere mantener Caja abierta ni configurar otro cron; el endpoint cron autenticado puede usarse como respaldo.

El estado se guarda en `ConfiguracionGlobal` bajo claves `mp:reportes:auto:v1:<cuenta>`, incluyendo período pendiente, avance por fila y pendientes de revisión. Un lease mediante compare-and-swap coordina réplicas, botón y cron y vence en dos minutos. El primer recorrido recupera siete días; los siguientes avanzan hasta cinco minutos antes de la consulta con 48 horas de solapamiento. El avance persiste entre reinicios. Se consulta un reporte pendiente cada cinco minutos y se solicita nuevamente después de una hora si sigue sin aparecer; tras completar el período actual, se espera 15 minutos para el próximo. La demora real depende de la generación de MP. El worker procesa una fila por paso de 15 segundos, con llamadas HTTP acotadas.

Se verifica la cuenta mediante /users/me y se seleccionan archivos de esa cuenta y del período exacto. Se preservan las opciones existentes del reporte y se agregan columnas requeridas e inclusión de retiros cuando falta. HTTP 203, fallas de acceso o reportes malformados son errores visibles, nunca una actualización vacía exitosa. Los reportes admiten hasta 2 MB; los sin egresos también completan el período. La cola admite hasta 1000 pendientes y se detiene con error si requiere revisión.

Sólo SETTLEMENT negativos verificados por pago directo como approved, ARS y account_money se incorporan. Se exige ID, importe total pagado y fecha coincidentes; faltas de payer/collector se admiten por la evidencia del reporte, pero las contradicciones bloquean. No se usa el neto recibido por el destinatario. Cada creación de MP, movimiento, saldo y auditoría es atómica, Serializable y con bloqueo por mpId. La auditoría automática tiene actor nulo (sistema) y conserva ID y hash SHA256 del reporte.

Los candidatos manuales del mismo importe en mercado_pago dentro de ±3 días, registros MP sin vínculo, eventos repetidos, tipos no soportados y diferencias quedan pendientes sin descuento. Este criterio no detecta pagos agrupados ni cargados en otras cajas o fechas más alejadas. La automatización no concilia el saldo total de la billetera, ni revierte devoluciones, ni administra MP Juani.

Caja consulta el estado cada 15 segundos: última consulta, período revisado, incorporados y pendientes. **Revisar pendientes automáticos** abre una preview verificada de hasta 30 operaciones del mismo reporte. Se puede vincular una carga sugerida sin otro descuento; las decisiones requieren confirmación. **Conciliar reporte MP** conserva la importación manual de CSV de hasta 30 egresos para otros períodos. Ambas previews se firman por usuario/cuenta y vencen en 15 minutos; confirmar vuelve a validar el pago y las coincidencias dentro de la transacción.
