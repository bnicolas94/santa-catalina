...
**Recuperación por reporte:** el botón **Conciliar reporte MP** permite a ADMIN cargar un CSV settlement con hasta 30 egresos. La vista previa no escribe datos: verifica cuenta del token, pago aprobado ARS con saldo, importe y fecha contra la API. La evidencia de dirección es el importe negativo del reporte; no exige payer/collector si faltan, pero bloquea contradicciones. El plazo de 48 horas del cron no aplica a este flujo.

La vista previa busca movimientos manuales activos del mismo importe en `mercado_pago` dentro de ±3 días. No detecta pagos agrupados, otras cajas ni fechas más alejadas, por lo que requiere revisión del administrador. Todas las filas comienzan en **Omitir**. Se puede incorporar un nuevo egreso sólo sin candidatos o vincular uno sugerido sin volver a descontar saldo. Los movimientos ya registrados por ID o con diferencias frente a la API quedan bloqueados. Confirmar exige una vista previa firmada para el usuario/cuenta, vigente por 15 minutos; vuelve a comprobar la API y los movimientos internos. El lote es atómico, con aislamiento Serializable y bloqueo por mpId compartido con cron. La auditoría conserva el ID de pago y hash SHA256 del archivo. Este flujo recupera y concilia el reporte confirmado; no amplía la cobertura automática de Payments Search.
