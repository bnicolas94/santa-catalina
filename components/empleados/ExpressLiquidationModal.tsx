...
export function ExpressLiquidationModal({ empleado, onClose, onSuccess }: ExpressLiquidationModalProps) {
    ...
    const [modeloRecibo, setModeloRecibo] = useState<ModeloRecibo>('B')
    ...
    const handleGuardarYImprimir = async () => {
        ...
        if (!ventanaImpresion) {
            alert('El navegador bloqueó la ventana de impresión. Habilitá las ventanas emergentes e intentá nuevamente.')
            return
        }
        ventanaImpresion.document.write('<p style="font-family:sans-serif;padding:24px">Guardando liquidación y preparando recibo…</p>')
        ...
        try {
            ...
            if (res.ok) {
                ...
                await printRecibo(liq, ventanaImpresion)
                onSuccess()
                onClose()
            } else {
    ...
    const printRecibo = async (liq: LiquidacionExpressCreada, ventanaImpresion: Window) => {
        const licenciaActivaSeleccionada = tiposLicencias.find(l => l.id === licenciaId)
        await imprimirRecibosLiquidacion([{
            ...
            tipo: 'NORMAL',
            horasExtras: Number(horasExtras) || 0,
            sueldoProporcional: valSueldo,
            montoHorasExtras: valExtras,
            montoAdicionales: adicionales.reduce((total, adicional) => total + adicional.montoCalculado, 0),
            descuentos: Number(liq.descuentosPrestamos) || 0,
            totalNeto: Number(liq.totalNeto) || 0,
            conceptos: adicionales.map(adicional => ({
                nombre: adicional.nombre,
                monto: adicional.montoCalculado,
                detalle: 'Carga manual Express',
            })),
            licenciaNombre: licenciaActivaSeleccionada?.nombre,
        }], modeloRecibo, ventanaImpresion)
    }

    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal" onClick={e => e.stopPropagation()} style={{ width: '500px', maxWidth: '95vw', maxHeight: '92vh', display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', overflowY: 'auto' }}>
                    ...
                    <div className="form-group">
                        <label className="form-label">Modelo del recibo</label>
                        ...
                        <span style={{ fontSize: 'var(--text-xs)', color: 'var(--color-gray-500)', marginTop: '4px', display: 'block' }}>
                            {MODELOS_RECIBO.find(modelo => modelo.value === modeloRecibo)?.detalle}
                        </span>
                    </div>
                ...
                <div className="modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: 'var(--space-3)', flexShrink: 0 }}>
                    ...
                    <button className="btn btn-primary" disabled={guardando || prestamosLoading || Boolean(prestamosError)} onClick={handleGuardarYImprimir}>
                        {guardando ? 'Guardando...' : `Guardar e imprimir · Modelo ${modeloRecibo}`}
                    </button>
                </div>
            </div>
            <style jsx>{`
