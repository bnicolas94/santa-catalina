    ...
    return <div style={{ backgroundColor: 'white', padding: 'var(--space-2)', borderRadius: 'var(--radius-sm)', border: `1px solid ${dia.esFeriado ? 'var(--color-warning)' : 'var(--color-gray-200)'}`, fontSize: '11px', opacity: dia.horasTrabajadas > 0 ? 1 : 0.5 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '2px' }}>
                    <input type="time" className="form-input" style={{ width: '52px', padding: '0px 1px', fontSize: '9px', height: '18px', textAlign: 'center', border: '1px solid var(--color-gray-200)', borderRadius: '2px' }} value={toTimeInputValue(dia.entrada)} onChange={e => props.onTimeChange(empleadoId, dia.fecha, 'entrada', e.target.value)} title="Hora de entrada" />
                    <span style={{ fontSize: '9px', color: 'var(--color-gray-400)' }}>a</span>
                    <input type="time" className="form-input" style={{ width: '52px', padding: '0px 1px', fontSize: '9px', height: '18px', textAlign: 'center', border: '1px solid var(--color-gray-200)', borderRadius: '2px' }} value={toTimeInputValue(dia.salida)} onChange={e => props.onTimeChange(empleadoId, dia.fecha, 'salida', e.target.value)} title="Hora de salida" />
                </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                {dia.horasTrabajadas === 0 && <button className="btn btn-ghost" title="Justificar día completo" onClick={() => props.onJustificar(empleadoId, dia.fecha)} style={{ padding: '2px', height: 'auto', fontSize: '14px', color: 'var(--color-success)' }}>🟢</button>}
                {dia.esJustificado && <button className="btn btn-ghost" title="Quitar justificación" onClick={() => props.onQuitarJustificacion(empleadoId, dia.fecha)} style={{ padding: '2px', height: 'auto', fontSize: '14px', color: 'var(--color-danger)' }}>🔴</button>}
            </div>
        </div>
        <div style={{ marginTop: '4px', borderTop: '1px dashed var(--color-gray-200)', paddingTop: '4px', marginBottom: '4px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {props.actualizandoEstado ? <span style={{ fontSize: '8px', color: 'var(--color-gray-400)' }}>Guardando...</span> : <select value={props.estado} onChange={e => props.onStatusChange(empleadoId, dia.fecha, e.target.value)} className="form-select" style={{ padding: '0px 2px', fontSize: '9px', height: '16px', width: '80px', border: '1px solid var(--color-gray-200)', borderRadius: '2px', cursor: 'pointer' }}>
                    <option value="TRABAJO">🟢 Trabajó</option><option value="FRANCO">⚪ Franco</option><option value="FERIADO">🚩 Feriado</option><option value="ENFERMEDAD">🟣 Enfermedad</option><option value="SIN_AVISO">🔴 Sin Aviso</option><option value="CON_AVISO">🟠 Con Aviso</option>
                </select>}
            </div>
        </div>
