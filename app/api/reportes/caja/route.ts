...
import { getCajaReport } from '@/lib/services/reportes-caja'
import { tienePermisoEnSesion } from '@/lib/auth/permisosSesion'

export async function GET(request: Request) {
    try {
        const session = await getServerSession(authOptions)
        if (!tienePermisoEnSesion(session, 'permisoReportes') && !tienePermisoEnSesion(session, 'permisoCaja')) {
        ...
        const data = await getCajaReport(desdeIso, hastaIso, ubicacionId)

        return NextResponse.json(data)
    } catch (error) {
        console.error('Error calculando reporte caja:', error)
        return NextResponse.json({ error: 'Error calculando reporte de caja' }, { status: 500 })
    }
}
