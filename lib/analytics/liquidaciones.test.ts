...
test('normaliza las clases de liquidación sin mezclar sueldo, SAC y vacaciones', () => {
    ...
    assert.equal(normalizarTipoLiquidacion('FERIADO_ADEUDADO'), 'FERIADO_ADEUDADO')
...
test('agrupa importes y cantidades por naturaleza de liquidación', () => {
    const grupos = agruparLiquidacionesPorTipo([
        ...
        { tipo: 'FERIADO_ADEUDADO', totalNeto: 10_000 },
    ])
    ...
    assert.equal(grupos.find(grupo => grupo.tipo === 'FERIADO_ADEUDADO')?.total, 10_000)
})
