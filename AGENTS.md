...
- `/produccion/pantalla` y su API requieren el permiso independiente `permisoPantallaProduccion` (o ADMIN); el rol `ADMIN_OPS` y los roles que ya tenían Producción lo reciben al migrar, y después puede revocarse por rol. Consulta cada 30 segundos los totales de `Paq. Totales` del Excel de OneDrive configurado por `PRODUCCION_PEDIDOS_EXCEL_URL`; el servidor reutiliza la lectura durante dos minutos. Toma una foto del stock de fábrica al primer acceso desde las 9:00 de Argentina. Cuenta aparte los movimientos de lotes desde las 9:00, incluso los anteriores a la foto; los descuenta de la base si ya estaban incluidos, salvo cuando una corrección manual `ajuste` posterior establece una nueva base para esa presentación. Aplica los ajustes posteriores a la foto y resta los pedidos de los tres turnos aunque oculte las filas de turnos finalizados. Los turnos son Mañana 9:00–13:00, Siesta 13:00–16:00 y Tarde 16:00–21:00; cada fila se oculta al terminar su turno. No escribe en el Excel ni crea movimientos de stock.
