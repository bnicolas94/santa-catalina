...
- La ficha rápida del pedido vive en `Conversation` dentro de `crm`: fecha como `YYYY-MM-DD` local, dirección, modalidad, turno y el ID externo/nombre instantáneo del local de retiro. Los puntos de retiro se consultan por API interna desde ubicaciones activas de tipo `LOCAL` del ERP. Sólo el agente dueño del lease puede editarla. `Agendado` fotografía la ficha en `ScheduledOrder` con idempotencia y auditoría, luego limpia el borrador; representa la carga manual en Excel y no crea un `Pedido` en el ERP. El historial resuelve nombres de agentes por una API interna del ERP y conserva un respaldo visual si esa consulta falla.
