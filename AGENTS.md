...
> Última verificación: 2026-09-23. Este es el punto de entrada para trabajar en el repositorio sin relevar la aplicación completa en cada tarea.
...
- El diario `/diario-errores` y sus APIs requieren `permisoDiarioErrores` o ADMIN. Los usuarios habilitados consultan y reportan todo el diario; sólo el autor o ADMIN puede editar un registro, con control de versión. Sólo ADMIN configura áreas (nombre y estado); desactivarlas conserva el historial. Los registros guardan fecha local `YYYY-MM-DD`, responsable libre y autor de la carga. `nombreCliente` es texto opcional para incidentes de atención, visible en el historial, buscador y CSV; está disponible sin depender del nombre configurable del área. Omitirlo conserva el valor al editar desde clientes anteriores; enviarlo vacío lo borra. El check `solucionado` guarda explícitamente si se pudo resolver el incidente, independientemente del texto opcional de solución. Los históricos sin check quedan `null` (sin confirmar); nunca se infiere el resultado del texto. Clientes anteriores que omiten el campo conservan el estado al editar. El resumen agrega total, solucionados, no solucionados y sin confirmar en la base con los mismos filtros del listado, sin limitarse a la página visible. El CSV incluye el estado, respeta los filtros y neutraliza fórmulas; rechaza exportaciones de más de 10.000 registros para solicitar un rango menor.
...
- La liquidación semanal asigna una caja de pago por empleado, sugiriendo una caja activa de su sede y permitiendo corregirla antes de confirmar. Cada liquidación genera su propio egreso en la caja elegida; el selector global sólo actúa como valor predeterminado y nunca reescribe pagos históricos. Tanto el jornal proporcional como las horas extras automáticas se calculan al minuto real; las extras no se redondean a bloques de media hora.
- Los horarios rotativos se resuelven en `lib/rrhh/horarios.ts`: excepción por fecha → plantilla semanal con vigencia → horario habitual. Plantillas y excepciones viven en modelos independientes; ADMIN las administra desde Turnos / Horarios por empleado. No se permite modificar períodos pagados o cerrados ni aplicar plantillas retroactivas. Nómina e importación comparten esta planificación para horas esperadas y tardanzas; no reescriben fichadas ni liquidaciones históricas.
- La grilla `PlanificadorSemanal` guarda excepciones diarias y copia la semana anterior primero como vista previa, sin duplicar fichadas. `esFranco` explícito evita tardanzas y ausencias automáticas; sin marcas no devenga jornal, con marcas conserva el cálculo de trabajo para revisión. Días sin planificación mantienen el horario habitual. Cada día se confirma independientemente y los rechazos permanecen pendientes y visibles.
...
- Los depósitos de locales mueven el importe al declararse desde la Caja Chica hacia la caja activa de la misma sede marcada como `recibeDepositos` (Caja Fuerte). El sobre queda pendiente allí; al validarlo, se ajustan diferencias con contrapartida en ambas cajas. ADMIN puede transferir el monto real a otra caja o dejarlo en la Caja Fuerte, sin crear otra entrada de dinero. Los usuarios que no son ADMIN pueden ver la Caja Fuerte, pero no registrar, editar, anular, ajustar ni transferir movimientos directos en ella; sólo pueden acreditarla mediante este circuito de depósito. No pueden depositar más que el saldo disponible de la Caja Chica salvo que un ADMIN activo autorice en el momento con sus credenciales; la autorización queda en la auditoría del movimiento. Los depósitos históricos sin caja de recepción conservan su conciliación anterior y no ofrecen la opción de dejar dinero en Caja Fuerte.
- Los indicadores diarios de ingresos, egresos y saldo de `/caja` representan flujo externo. Excluyen transferencias entre cajas y todos los movimientos vinculados al circuito de depósitos; esos movimientos permanecen visibles en el historial y siguen afectando el saldo individual de cada caja.
- Los egresos de MP se sincronizan desde reportes settlement en `lib/services/mercadopago-automatico.service.ts`: `npm start` inicia el worker vía `instrumentation.ts`, independiente de Caja. Estado, checkpoint y lease se guardan en `ConfiguracionGlobal` por cuenta. Recupera inicialmente 7 días y luego avanza con solapamiento de 48 h. Sólo SETTLEMENT negativos verificados como pagos aprobados ARS/account_money, con importe y fecha coincidentes, se incorporan con saldo/auditoría y deduplicación por mpId; posibles cargas manuales quedan pendientes para preview y confirmación ADMIN. Tipos no soportados o diferencias no descuentan y se muestran como pendientes. Cron y POST `/api/mercadopago/sincronizar` comparten el proceso; GET consulta el estado. La búsqueda antigua `lib/mercadopago-egresos.ts` queda como utilitario, sin programación activa.
- Las ventas del Google Sheet externo se sincronizan en `lib/services/google-sheets-caja.service.ts`; `instrumentation.ts` inicia un worker independiente, salvo `SHEETS_CAJA_AUTO=false`. ADMIN configura `/cajas/google-sheets`: cada texto de Ubicación se vincula a una sede y a cajas distintas para Efectivo y Transferencia. Sólo filas posteriores a la activación con Estado `Entregado`, medio soportado, fecha e importe válidos crean ingresos. `MovimientoSheetCaja` deduplica por spreadsheet/hoja/ID y vincula el movimiento atómicamente; una corrección posterior queda para revisión y nunca revierte dinero automáticamente. El Sheet se lee mediante enlace público y no se modifica.
- Los cambios de stock y de producto deben conservar trazabilidad hacia compras, lotes o pedidos según corresponda.
- `/produccion/pantalla` y su API requieren el permiso independiente `permisoPantallaProduccion` (o ADMIN); el rol `ADMIN_OPS` y los roles que ya tenían Producción lo reciben al migrar, y después puede revocarse por rol. Antes de las 9:00 de Argentina muestra la espera sin depender del Excel ni de la base. Desde las 9:00 consulta cada 30 segundos los totales de `Paq. Totales` del Excel de OneDrive configurado por `PRODUCCION_PEDIDOS_EXCEL_URL`; el servidor reutiliza la lectura durante dos minutos y reintenta una vez ante fallas transitorias de red o del proveedor. ADMIN ve un detalle técnico acotado cuando falla la lectura. Toma una foto del stock conjunto de fábrica y sucursales al primer acceso desde las 9:00 de Argentina. Las fotos anteriores que sólo contienen fábrica incorporan el stock de sucursales reconstruido a la hora original mediante movimientos posteriores, sin reescribir la foto guardada. Cuenta aparte los movimientos de lotes desde las 9:00, incluso los anteriores a la foto; los descuenta de la base si ya estaban incluidos, salvo cuando una corrección manual `ajuste` posterior establece una nueva base para esa presentación. Aplica los ajustes posteriores a la foto y resta los pedidos de los tres turnos aunque oculte las filas de turnos finalizados. Los turnos son Mañana 9:00–13:00, Siesta 13:00–16:00 y Tarde 16:00–21:00; cada fila se oculta al terminar su turno. No escribe en el Excel ni crea movimientos de stock.
- La pantalla de stock proyecta las fechas disponibles en `Paq. Totales` hasta 30 días hacia adelante. El stock inicial de cada fecha futura es el stock libre de la fecha anterior; la producción futura vale cero hasta que ese día pase a ser el actual y el ERP registre producción real. El día actual sigue tomando el stock y los movimientos reales del ERP.
- En `/produccion/pantalla`, los traslados entre fábrica y sucursales no modifican el stock conjunto: no se reconstruyen en la base ni se suman como entradas. Los envíos desde Fábrica cubren primero los pedidos del Excel del día; sólo el excedente se aparta del stock libre como mercadería destinada a venta espontánea.
- Los lotes de hoy aún `en_produccion` se muestran aparte por presentación como paquetes en proceso; no ingresan en «Producido hoy» ni en el stock libre hasta que el cierre del lote cree el movimiento de producto terminado. Junto al stock libre se muestra un teórico igual al libre más esos paquetes abiertos, identificado como estimación; las fechas futuras siguen encadenando el stock libre confirmado, no el teórico. El total de la página general de Producción incluye esos lotes y debe identificarse como paquetes en lotes.
- La ropa de trabajo usa un stock general por prenda (Remera/Buzo) y talle. Sólo ADMIN puede consultar o modificar stock, talles, entregas, configuración y constancias. Cada entrega nueva descuenta stock y deja movimientos en una transacción; anularla restituye sus prendas con motivo y auditoría. Las nuevas entregas fotografían prenda, talle, cantidad, modelo, marca y certificación. La constancia SRT 299/11 reúne entregas por trabajador, deja una firma en papel por renglón y toma razón social/CUIT, domicilio por sede y EPP requeridos por puesto desde la configuración del módulo. No se imprime si faltan datos obligatorios; imprimir nunca crea una entrega. Los registros anteriores al circuito de stock permanecen como historial sin descuento retroactivo ni talle reconstruido.
- `/reportes/pedidos-local` y su API requieren `permisoReportes`. Consultan las hojas configuradas en Caja con caché de lectura de dos minutos, sin escrituras ni límite de activación de la sincronización. `lib/reportes/pedidos-local.ts` cuenta un ticket por hoja/ID y agrupa por `Fecha/Hora` de registro en Argentina, no por hora de cobro o entrega. Por defecto muestra pedidos comunes entregados; informa cobertura parcial, duplicados y datos inválidos. No reemplaza los reportes contables de Caja.
...
- Las imágenes guardan `mediaId` y el enlace completo `mediaUrl` de YCloud. El navegador sólo usa la ruta autenticada `/api/conversations/[id]/messages/[messageId]/image`, con visibilidad de conversación, descarga en servidor, dominios/formato/tamaño restringidos y sin devolver credenciales ni enlaces firmados del proveedor. YCloud conserva los archivos por un período limitado; un archivo vencido muestra un aviso con reintento.
...
- La bandeja Nuevas y su contador usan `assignedToId=null` y excluyen resueltas/archivadas, incluso si un registro histórico quedó abierto o en espera. Recibir un mensaje entrante mantiene `UNASSIGNED` mientras no haya dueño y abre el chat para el mismo agente cuando ya está asignado; no reasigna por recepción.
...
- **Conciseness**: Give short answers. Do not explain standard code or logic unless explicitly asked.
- **Diffs Only**: Never rewrite entire files. Only output the exact lines or blocks that need changes (use standard unified diff format or comments).
- **No Boilerplate**: Omit imports, structural code, or setup logic if it hasn't changed.

## Scope & Ignored Files
- **Ignore**: node_modules/, dist/, build/, .git/, package-lock.json, yarn.lock, *.log, telemetry charts.
- **Focus**: Only analyze the files explicitly mentioned in the user prompt. Do not read the entire directory unless necessary.
