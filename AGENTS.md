...
- La ficha rápida del pedido vive en `Conversation` dentro de `crm`: fecha como `YYYY-MM-DD` local, dirección, modalidad, turno, estado de pago confirmado, observaciones, productos/presentaciones externas del ERP con cantidades e instantáneas descriptivas, y el ID externo/nombre instantáneo del local de retiro. Catálogo, puntos de retiro, dirección actual y pedidos históricos se consultan exclusivamente por APIs internas del ERP; el detalle de un pedido se muestra sólo si pertenece al cliente vinculado. Sólo el agente dueño del lease puede editar la ficha. `Agendado` fotografía la ficha en `ScheduledOrder` con idempotencia y auditoría, luego limpia el borrador; representa la carga manual en Excel y aún no crea un `Pedido` en el ERP. El historial resuelve nombres de agentes por una API interna del ERP y conserva un respaldo visual si esa consulta falla.
